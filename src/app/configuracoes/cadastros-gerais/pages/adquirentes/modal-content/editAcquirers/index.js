...
const RegisterAcquirers = ({ onClose, acquirers, findData }) => {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      codigoInterno: acquirers.codigo_interno,
      nomeDoAdquirente: acquirers.nome_adquirente,
      descricao: acquirers.descricao,
      contaAssociada: {
        id: acquirers.conta_bancaria.id,
        label: `${acquirers.conta_bancaria.banco.nome} - ${acquirers.conta_bancaria.observacoes} - ${acquirers.conta_bancaria.agencia}-${acquirers.conta_bancaria.digito_agencia}/${acquirers.conta_bancaria.numero_conta}-${acquirers.conta_bancaria.digito_conta}`,
      },
      unidadeAssociada: {},
      unidadeAssociadasSelecionadas: acquirers.unidades_associadas.map(
        (item) => {
          return {
            id: item.unidade_saude.id,
            label: item.unidade_saude.nomeFantasia,
          }
        },
      ),
      cartaoSuportado: {},
      cartoesSuportadosSelecionados: acquirers.tipos_cartao.map((item) => {
        return {
          id: item.tipo_cartao.id,
          label: item.tipo_cartao.textoAlternativa,
        }
      }),
      opcaoDeParcelamento: {
        id: acquirers.opcao_parcelamento.id,
        label: acquirers.opcao_parcelamento.textoAlternativa,
      },
      taxaPorTransacao: acquirers.taxa_transacao,
      taxaPorParcelamento: acquirers.taxa_parcelamento,
      porcentagemDeRepasse: acquirers.percentual_repasse,
      prazoDeRepasse: acquirers.prazo_repasse,
      restricoes: acquirers.restricoes.map((item) => {
        return {
          unidade: {
            id: item.unidade_saude.id,
            label: item.unidade_saude.nomeFantasia,
          ...
          restricao: {
            id: item.restricao.id,
            label: item.restricao.textoAlternativa,
          },
        }
      }),
    },
    onSubmit: async (values, { setSubmitting }) => {
      try {
        const payload = {
          ...
          taxa_transacao: Number(values.taxaPorTransacao),
          taxa_parcelamento: Number(values.taxaPorParcelamento),
          percentual_repasse: Number(values.porcentagemDeRepasse),
        ...
        try {
          const responseCreateUnity = await UpdateAcquire(payload, acquirers.id)
