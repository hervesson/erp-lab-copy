...
const CustomSelect = ({
  ...
  const toggleSelect = (item) => {
    // Se já existe → remove
    if (select.some((i) => i.id === item.id)) {
      setSelect(select.filter((i) => i.id !== item.id))
    }
  ...
  return (
    <div style={{ position: 'relative', flex: 1 }}>
      <div
        className={`flex h-10 items-center justify-between rounded-lg px-2 ${className}`}
        ...
        >
          {options.map((option, index) => (
            ...
            >
              <input
                type="checkbox"
                checked={select.some((i) => i.id === option.id)}
                readOnly
              />
              <p
